...
const AddListing = () => {
    ...
    const getListingDetails = async () => {
        const res = await db.select().from(CarListing).innerJoin(CarImages,eq(CarListing.id,CarImages.carListingId))
    ...
    const onSubmit = async (e) => {
        ...
        toast({
          ...
          if(mode == 'edit') {
            const res = await db.update(CarListing).set({
                ...formData,
        ...
        try {
            const res = await db.insert(CarListing).values({
                ...formData,
                features: featuresData,
                createdBy:user?.primaryEmailAdress?.emailAdress,
                postedOn: new Date().toISOString()
    ...
    <div>
        ...
        <div className='px-10 md:px-20 my-10'>
            ...
            <form className='p-10 border rounded-xl mt-10'>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-3 gap-2'>
                        {features.features.map((item, index) => (
                            <div key={index} className='flex gap-2 items-center'>
                                ...
                                checked={featuresData?.[item?.name]} /> <h2>{item.label}</h2>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* car images */}
                <Separator className="my-6" />
                <UploadImages mode={mode} carInfo={carInfo} triggerUploadImages={triggerUploadImages} setLoader={(v)=>{setLoader(v); }}/>

                <div  className="mt-10 flex justify-end">
                    <Button type="submit" disabled={loader} onClick={(e)=>onSubmit(e)}>
                        {loader ? <BiLoaderAlt className='animate-spin text-lg' /> : 'Submit'}</Button>
                </div>
            </form>
